...
class TeamService {
    ...
    // Fetch only approved teams asynchronously using async/await
    func fetchApprovedTeamsAsync() async throws -> [TeamData] {
        let snapshot = try await db.collection("teams")
                                  .whereField("isApproved", isEqualTo: true)
                                  .getDocuments()
        
        print("📊 Found \(snapshot.documents.count) approved teams")
        ...
        let teams = snapshot.documents.compactMap { document -> TeamData? in
            ...
            print("✅ Approved team '\(name)' has \(players.isEmpty ? "NO" : "\(players.count)") players: \(players)")
    ...
    // Fetch approved teams using completion handler
    func fetchApprovedTeams(completion: @escaping ([TeamData]) -> Void) {
        db.collection("teams")
          .whereField("isApproved", isEqualTo: true)
          .getDocuments { snapshot, error in
            guard error == nil else {
                print("❌ Error fetching approved teams: \(error!.localizedDescription)")
                completion([])
                return
            }
            
            guard let documents = snapshot?.documents else {
                print("⚠️ No approved team documents found")
                completion([])
                return
            }
            
            print("✅ Successfully loaded \(documents.count) approved teams")
